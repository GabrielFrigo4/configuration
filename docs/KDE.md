...
A interface gráfica é o componente mais pesado que tem a permissão de rodar diretamente no *Host* (conforme as diretrizes de [Filosofia](PHILOSOPHY.md)). No **FreeBSD Desktop**, adotamos o **KDE Plasma 6** com **Wayland** nativo por oferecer a melhor integração com hardware, customização absoluta e aceleração gráfica no ecossistema BSD.
...
- **Caminho do Agente no FreeBSD:**
  ...
  ```
  *(Diferente do Linux, onde fica em `/usr/lib/...`, no FreeBSD softwares de terceiros residem estritamente em `/usr/local/`)*.
