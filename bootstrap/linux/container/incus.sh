...
if command -v dnf > "/dev/null" 2>&1; then
	${SUDO} dnf install --assumeyes incus 2> "/dev/null" || true
elif command -v apt > "/dev/null" 2>&1; then
	${SUDO} apt install --yes incus lxc 2> "/dev/null" || true
...
${SUDO} usermod --append --groups incus-admin "${TARGET_USER}" 2> "/dev/null" || true
